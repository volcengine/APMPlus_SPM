...
DSYM_UPLOAD_URL="https://console.volcengine.com/apmplus_api/eue/guest/app/mapping/upload"
...
function main() {
    ...
    function uploadDSYM() {
        ...
        echo "curl ${DSYM_UPLOAD_URL} -F \"file=@${DSYM_ZIP_FPATH}\"  -F \"aid=${APMPlus_APP_ID}\" -H \"Content-Type: multipart/form-data\" -w %{http_code} -v "
            
        echo "--------------------------------"
            
        # 上传请求
        STATUS=$(curl ${DSYM_UPLOAD_URL} -F "file=@${DSYM_ZIP_FPATH}" -F "type=Dwarf" -F "os=iOS" -F "aid=${APMPlus_APP_ID}" -H "Content-Type: multipart/form-data" -w %{http_code} -v)
